...
Var
  ...
  long gyro[3]
  long timerStack[100], debugStack[100], sensorStack[100]
...
OBJ
  ...
  sensor  : "DCM_Tier1_Gyro.spin"
PUB main | insert_card, text, now
  ...
  sd.popen(string("output1.txt"), "w")  
  debug.str(0, String("File Opened"))

  cognew(runSensor, @sensorStack)
  ...
  repeat while flag > 0
    ...
    sd.newline
    now := cnt
    repeat while (cnt < now + (clkfreq/176-(now-base)))
...
PUB runDebug
  repeat while flag > 0
    ...
    debug.str(0,String("gyro[0]: "))
    debug.decLn(0,gyro[0])
    debug.str(0,String("gyro[1]: "))
    debug.decLn(0,gyro[1])
    debug.str(0,String("gyro[2]: "))
    debug.decLn(0,gyro[2])
    debug.newline(0)      
    waitcnt(cnt + clkfreq/10)
